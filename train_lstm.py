...
# Function to train LSTM on a single coin dataset
def train_lstm_on_coin(file_path, time_step=60, epochs=20, batch_size=32, verbose=1):
    coin_name = os.path.basename(file_path).replace("_scaled.csv", "")
    print(f"\n🚀 Training LSTM for: {coin_name}")
